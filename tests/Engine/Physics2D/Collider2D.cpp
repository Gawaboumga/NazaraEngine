...
SCENARIO("Collider2D", "[PHYSICS2D][COLLIDER2D]")
{
	GIVEN("No particular elements")
	{
        ...
        WHEN("We construct a circle")
        {
            ...
            THEN("We expect those to be true")
            {
                Nz::Rectf aabb(position.x - radius, position.y - radius, 2.f * radius, 2.f * radius);
                CHECK(circle.GetAABB() == aabb);
        ...
        WHEN("We construct a null")
        {
            ...
            THEN("We expect those to be true")
            {
                CHECK(null.GetAABB() == Nz::Rectf::Zero());
        ...
        WHEN("We construct a segment")
        {
            ...
            THEN("We expect those to be true")
            {
                Nz::Rectf aabb(-4.f, -3.f, 6.f, 4.f);
                CHECK(segment.GetAABB() == aabb);
